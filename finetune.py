"""
To resume a learning or fine-tune(addtitional training) a model.
"""
...
data_dir = "/mnt/nfs-mnj-hot-02/tmp/i23_eric/i23_eric/Data/pretrain/sequences_30M_len512" 
ckpt_path = "" # checkpoint path. the model for additional training
out_dir = '/mnt/nfs-mnj-hot-02/tmp/i23_eric/i23_eric/Data/out/30M_len512_2'  # output directory
log_dir = os.path.join(out_dir, 'logs') # logs will be written in to out_dir/logs

# -----------------------------------------------------------------------------
# model parameters
meta_vocab_size = 2048 
block_size = 1024
n_layer = 12
n_head = 12
n_embd = 768
...
gradient_accumulation_steps = 8 # used to simulate larger batch sizes, should be multiple of GPU number
...
lr_decay_iters = 800000 # should be ~= max_iters per Chinchilla
...
if "OMPI_COMM_WORLD_SIZE" in os.environ:
    ...
    # os.environ['LOCAL_RANK'] = os.environ['OMPI_COMM_WORLD_LOCAL_RANK'] #搞不清WORLD LOCAL RANK是个什么
else:
    ddp = False
...
if ddp:
    ...
    if gradient_accumulation_steps != 0 and gradient_accumulation_steps % ddp_world_size != 0:
...
log_and_write(log_dir, f"tokens per iteration will be: {tokens_per_iter:,}")
...
if init_from == 'resume' and inheritance_optimizer == True:
    optimizer.load_state_dict(checkpoint['optimizer'])
    print("optimizer_resumed")
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        log_and_write(log_dir, f"step {iter_num}: train loss {losses['train']:.4f}, val loss {losses['val']:.4f}")
        if iter_num % 200 == 0:
